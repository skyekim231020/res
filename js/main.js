$(document).ready(function(){
  // 네비게이션(GNB)
  // 만약 윈도우의 가로길이가 1280이상이면 PC버전으로 메뉴가 나타나고, 윈도우의 가로길이가 1280 미만이면 모바일 버전 메뉴가 나타남
  if($(window).width() >=1280){
    ...
    $('nav > ul > li > a').click(function(){
      // 태그의 다음 객체인 ul.sub가 slideToggle됨
    ...
    // header의 메뉴 버튼을 클릭하면 네비게이션 나타나고, 다시 클릭하면 네비게이션 들어감
    let menu_sw=0;
  ...
  // 자바스크립트 변수 선언할 때
  // let $header=$('header')
  // 선언하면 header 객체의 값과 속성 모두 저장하는 변수
  
  // let header = $('header').height();
  // 선언하면 header객체의 값(높이)만 저장하는 변수
  ...
  function minimize_header() {
    ...
    function has_scrolled() {
      ...
      if (current_scroll > last_scroll) {
        ...
        // <easing> ease-in: 가속운동, ease-out: 감속운동, ease-in-out: 가숙 후 감속운동, linear:등속운동
  ...
  // ★★★★★★★★★★★★★★★★★★★★★★★★ ios 플러그인 / 매직스크롤 플러그인 => 동일한 효과 낼 수 있는 플러그인들 ('~사용법 코드펜'으로 검색해서 사용법 익힐 수 있음)
  // 스크롤 내릴 때마다 section안의 자식 콘텐츠들이 서서히 나타나는 애니메이션
  $(window).scroll(function(){
    // 윈도우의 scrollTop값을 쟈ㅜ쌔ㅔqustndp wjwkd
    let winTop=$(this).scrollTop();
    // .scrollSet 영역 갯수만큼 반복하는 메서드(each())
    $('.scrollSet').each(function(){
      let sec2=$(this).offset().top;{}
  ...
  //   }
  // }).from('.time', { y:100, opacity:0, duration:0.5 })
  // .to('.time', { y:0, opacity:1, duration:0.5 });  
  ...
  $('.family_site > a ').click(function(e){
    e.preventDefault();
    // 만약 sw변수의 값이 0이면 .family_list 나타나고 ,
    // sw변수의 값을 1로 변경
    if(sw==0){
      sw=1;
      $('.family_list').slideDown();
      $('.family_site > a ').addClass('active');
      // sw변수의 값이 0이 아니면 .family_list 들어가고
      // sw변수의 값을 0으로 변경
    }else{
      sw=0;
      $('.family_list').slideUp();
...
//   let sw=0,
//   // family_site
//   $('.family_site > a').click(functin(e){
//     // 만약 sw변수의 값이 0이면 .family_lsit 나타나고, sw변수의 값을 1로 변경
//     e.preventDefault();
